...
describe("frozen-install", () => {
    ...
    it("should detect Node.js projects", () => {
      ...
      try {
        const result = execSync(
          `node ${join(__dirname, "../dist/frozen-install.js")} . true`,
          {
            encoding: "utf8",
            stdio: "pipe",
          },
  ...
  describe("ignore-scripts parameter", () => {
    it("should always ignore scripts regardless of input (legacy compatibility)", () => {
      writeFileSync(
        "package.json",
        '{"name": "test", "scripts": {"postinstall": "echo harmful"}}',
      );
      ...
      try {
        ...
        expect(error.stdout).toContain(
          "Ignore scripts: true (hardcoded for security)",
        );
      }
    });
  ...
  describe("file validation", () => {
    it("should validate compiled JavaScript exists", () => {
      expect(existsSync(join(__dirname, "../dist/frozen-install.js"))).toBe(
        true,
      );
    });
  });
