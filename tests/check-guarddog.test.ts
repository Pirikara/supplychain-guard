...
describe("check-guarddog", () => {
  ...
  describe("rules and configuration", () => {
    it("should handle all rules enabled by default", () => {
      ...
      expect(result).toContain(
        "Test environment detected, skipping GuardDog installation",
      );
    });
  ...
  describe("file validation", () => {
    it("should validate compiled JavaScript exists", () => {
      expect(existsSync(join(__dirname, "../dist/check-guarddog.js"))).toBe(
        true,
      );
    });
    ...
    it("should create valid JSON output", () => {
      ...
      try {
        ...
        console.log(
          "GuardDog execution failed, but this is acceptable for testing",
        );
      }
      ...
      const output = JSON.parse(
        require("node:fs").readFileSync("guarddog.json", "utf8"),
      );
